...
namespace BZFlag.Game
{
	public class GameTime
	{
		...
		protected double StepSecs = 0.0;
		protected Int64 StepTime = 0;
		...
		public void Update()
		{
			...
			else
			{
				...
				Int64 diffTime = StepTime - tr.NetTime;
		...
		public void SetStepTime()
		{
			...
			else
			{
				...
				double shortPred = (double)StepTime + (skipTime * AvgRate);

				// filtering
				double c = (skipTime * 1.0e-6) / FilterTime;
				double a = (c > 0.0) && (c < 1.0) ? c : 0.5;
				double b = 1.0 - a;
				StepTime = (Int64)((a * longPred) + (b * (double)shortPred));
			}
